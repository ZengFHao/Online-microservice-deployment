...
def calPareto(solution_set, new_solution):
    ...
    if all(not dominates(new_solution[0], sol[0]) for sol in solution_set) and new_solution[0] not in [row[0] for row in new_solution_set]:
...
def QPareto(solution_set):
    ...
    for new_solution in solution_set:
        ...
        if not is_dominated:
            # 如果新解不被支配，则将其添加到 Pareto 前沿
            pareto_set = [sol for sol in pareto_set if not dominates(new_solution[0], sol[0])]  #使用列表推导式和 dominates 函数，从 pareto_set 中移除所有被新解dominates的解
            if new_solution[0] not in [row[0] for row in pareto_set]:
                pareto_set.append(new_solution)
